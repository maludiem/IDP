    ...
    MESH_NODE<T, dim>& X, MESH_ELEM<2>& triangles)
{
    ...
    while (std::getline(is, line)) {
        ...
        if (line[0] == 'v' && line[1] == ' ') {
            ...
            if constexpr (dim == 2) {
                if (tri(0) < X.size && tri(1) < X.size && tri(2) < X.size) {
                    VECTOR<T, dim>& v0 = std::get<0>(X.Get_Unchecked(tri(0)));
                    VECTOR<T, dim>& v1 = std::get<0>(X.Get_Unchecked(tri(1)));
                    VECTOR<T, dim>& v2 = std::get<0>(X.Get_Unchecked(tri(2)));
                    if (cross(v1 - v0, v2 - v0) < 0) {
                        // mesh inverted
...
template <class T, int dim>
VECTOR<int, 4> Read_TriMesh_With_Tex_Obj(const std::string& filePath, 
    MESH_NODE<T, dim>& X, MESH_NODE<T, dim>& X_tex, 
    MESH_ELEM<2>& triangles, MESH_ELEM<2>& triangles_tex)
{
    std::ifstream is(filePath);
    if (!is.is_open()) {
        puts((filePath + " not found!").c_str());
...
template <class T, int dim>
py::array_t<T> Taichi_TriMesh_Obj(MESH_NODE<T, dim>& nodes, MESH_ELEM<2>& faces)
{
    auto output = py::array_t<T>(3 * (int)faces.size * 2);
    py::buffer_info buf = output.request();
    T* arr = (T*)buf.ptr;
    faces.Each([&](int id, auto data) {
        auto &[t] = data;
        arr[0 * (int)faces.size * 2 + id * 2 + 0] = std::get<0>(nodes.Get_Unchecked(t(0)))(0);
        arr[0 * (int)faces.size * 2 + id * 2 + 1] = std::get<0>(nodes.Get_Unchecked(t(0)))(1);
        arr[1 * (int)faces.size * 2 + id * 2 + 0] = std::get<0>(nodes.Get_Unchecked(t(1)))(0);
        arr[1 * (int)faces.size * 2 + id * 2 + 1] = std::get<0>(nodes.Get_Unchecked(t(1)))(1);
        arr[2 * (int)faces.size * 2 + id * 2 + 0] = std::get<0>(nodes.Get_Unchecked(t(2)))(0);
        arr[2 * (int)faces.size * 2 + id * 2 + 1] = std::get<0>(nodes.Get_Unchecked(t(2)))(1);
    });
    output.resize({3, (int)faces.size, 2});
    return output;
}

template <class T>
py::array_t<T> Taichi_SurfaceMesh_Obj(BASE_STORAGE<VECTOR<T, 3>>& nodes, BASE_STORAGE<int>& TriVI2TetVI, BASE_STORAGE<VECTOR<int, 3>>& faces)
{
    auto output = py::array_t<T>((int)faces.size * 3 * 3);
    py::buffer_info buf = output.request();
    T* arr = (T*)buf.ptr;
    faces.Each([&](int id, auto data) {
        auto [face] = data;
        for (int v = 0; v < 3; ++v)
            for (int d = 0; d < 3; ++d) {
                int i = std::get<0>(TriVI2TetVI.Get_Unchecked(face(v)));
                arr[id * 9 + v * 3 + d] = std::get<0>(nodes.Get_Unchecked(i))(d);
            }
    });
    output.resize({(int)faces.size, 3, 3});
    return output;
}

template <class T>
py::array_t<T> Taichi_DBC_Obj(VECTOR_STORAGE<T, 3>& DBC)
{
    auto output = py::array_t<T>((int)DBC.size * 2);
    py::buffer_info buf = output.request();
    T* arr = (T*)buf.ptr;
...
template <class T>
VECTOR<int, 4> Read_TetMesh_Tet(const std::string& filePath, MESH_NODE<T, 3>& X,
                                MESH_ELEM<3>& indices)
{
    std::ifstream in(filePath);
    if (!in.is_open()) {
    ...
    size_t n_points = 0;
    size_t n_tets = 0;
    size_t done_points = 0;

    while (std::getline(in, line)) {
        std::stringstream ss(line);
        if (line.size() == (size_t)(0)) {
        }
        else if (line.substr(0, 16) == "#num of vertex: ") {
            ss.ignore(128, ':'); // ignore "POINTS"
            ss >> n_points;
        }
        else if (line.substr(0, 20) == "#num of tetrahedra: ") {
            ss.ignore(128, ':'); // ignore "CELLS"
            ss >> n_tets;
            reading_points = true;
            reading_tets = false;
...
template <class T, bool mapTriVInd = true>
void Find_Surface_TriMesh_With_Extra_Mesh(
    BASE_STORAGE<VECTOR<T, 3>>& X,
    BASE_STORAGE<VECTOR<int, 4>>& Tet,
    BASE_STORAGE<int>& TriVI2TetVI, BASE_STORAGE<VECTOR<int, 3>>& Tri, MESH_ELEM<2>& Extra_Mesh)
{
    // indexing triangle faces
    std::map<Triplet, int> tri2Tet;
    Tet.Each([&](int id, auto data) {
    ...
    Extra_Mesh.Each([&](int id, auto data) {
      auto& [elemVInd] = data;
      tri2Tet[Triplet(elemVInd(0), elemVInd(1), elemVInd(2))] = id;
    });
...
VECTOR<int, 4> Merge_Heros(const VECTOR<int ,4>& hero0, const VECTOR<int, 4>& hero1)
{
    VECTOR<int, 4> all_heros;
    all_heros[0] = hero0[0];
    all_heros[1] = hero0[1];
    all_heros[2] = hero1[2];
    all_heros[3] = hero1[3];
...
void Export_MeshIO(py::module& m) {
    ...
    m.def("Write_TriMesh_Obj", &Write_TriMesh_Obj<double, 2>, "write planar triangle mesh to obj file");
    m.def("Write_TriMesh_Obj", &Write_TriMesh_Obj<double, 3>, "write triangle mesh to obj file");
    m.def("Taichi_TriMesh_Obj", &Taichi_TriMesh_Obj<double, 2>);
    m.def("Taichi_DBC_Obj", &Taichi_DBC_Obj<double>);
    m.def("Taichi_SurfaceMesh_Obj", &Taichi_SurfaceMesh_Obj<double>);
    m.def("Write_Surface_TriMesh_Obj_Regional", &Write_Surface_TriMesh_Obj_Regional<double>);
    m.def("Read_TetMesh_Vtk", &Read_TetMesh_Vtk<double>, "read tetrahedra mesh from vtk file");
    m.def("Read_TetMesh_Tet", &Read_TetMesh_Tet<double>, "read tetrahedra mesh from tet file");
