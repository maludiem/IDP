...
sys.path.append(r'/home/jiangjinke/IDP/IDP-modeling/build/JGSL.cpython-38-x86_64-linux-gnu.so')
import os
...
class FEMSimulationBase(SimulationBase):
    def __init__(self, precision, dim, model="FCR"):
        ...
        self.Xt = Storage.V2dStorage() if self.dim == 2 else Storage.V3dStorage()
        ...
        self.Elem_t = Storage.V3iStorage() if self.dim == 2 else Storage.V4iStorage()
        ...
        self.work_range = Vector4i(-1, -1, -1, -1)
        self.isOri = True
        self.origin_hand = Storage.V3dStorage()
        self.nearDistance = 0.0
        self.model_pos = Vector3d()
        self.WorkIndex = Storage.SiStorage()
        self.Xt_meshCounter = Vector4i()
        self.translate = Vector3d()
        self.rotCenter = Vector3d()
        self.rotAxis = Vector3d()
        self.rotDeg = 0.0
        self.scale = Vector3d()
    ...
    def write_to_file(self, filename):
        fn = "output/Ori.txt"
        MeshIO.Write_Ori(self.Xt,self.Elem_t,filename)
    ...
    def set_ori_hand(self, hand_position):
        MeshIO.Set_Ori_Hands(self.origin_hand,hand_position)

    def set_nearDistance(self,nearDistance, model_pos):
    ...
    def add_object(self, filePath, translate, rotCenter, rotAxis, rotDeg, scale, hero=False):
        ...
        if self.dim == 2:
            ...
            if filePath[-4:] == '.vtk':
                self.meshCounter = MeshIO.Read_TetMesh_Vtk(filePath, self.X0, self.Elem, self.Xt,self.Elem_t,
                                                           self.origin_hand, self.work_range,self.nearDistance,translate,
                                                           self.WorkIndex,self.Xt_meshCounter)
            if filePath[-4:] == '.tet':
                self.meshCounter = MeshIO.Read_TetMesh_Tet(filePath, self.X0, self.Elem)
        if hero:
            if self.hero_counter is None:
                self.hero_counter = self.Xt_meshCounter
            else:
                self.hero_counter = MeshIO.Merge_Heros(self.hero_counter, self.Xt_meshCounter)
        MeshIO.Transform_Points(translate, rotCenter, rotAxis, rotDeg, scale, self.meshCounter, self.X0)
        MeshIO.Transform_Points(translate, rotCenter, rotAxis, rotDeg, scale, self.Xt_meshCounter, self.Xt)
    ...
    def initialize_added_objects(self, velocity, p_density, E, nu):
        if self.dim == 3:
            MeshIO.Find_Surface_TriMesh(self.Xt, self.Elem_t, self.TriVI2TetVI, self.Tri)
        ...
        if self.enableFracture:
            FEM.Fracture.Initialize_Fracture(self.X0, self.Elem, self.debrisAmt, self.fractureRatio2,
                                             self.strengthenFactor, self.edge_dupV, \
                                             self.isFracture_edge, self.incTriV_edge, self.incTriRestDist2_edge)
        if self.EIPC:
            FEM.TimeStepper.ImplicitEuler.Initialize_Elastic_IPC(self.X0, self.Elem, self.dt, E, nu, self.dHat2,
                                                                 self.kappa)
    ...
    def set_plasticity(self, model_name, E, nu, yield_stress, fail_stress, xi):
        self.withPlasticity = True
        Set_Model_Parameter(model_name, "Von_Mises_E", E)
        Set_Model_Parameter(model_name, "Von_Mises_nu", nu)
        Set_Model_Parameter(model_name, "Von_Mises_yield_stress", yield_stress)
        Set_Model_Parameter(model_name, "Von_Mises_fail_stress", fail_stress)
        Set_Model_Parameter(model_name, "Von_Mises_xi", xi)
    ...
    def set_DBC(self, DBCRangeMin, DBCRangeMax, v, rotCenter, rotAxis, angVelDeg, scaleVel=0., durationStart=0,
                durationEnd=1e30):
        DBCMotion = Storage.V2iV2dV2dV2dSdSdStorage() if self.dim == 2 else Storage.V2iV3dV3dV3dSdSdStorage()
        FEM.Init_Dirichlet(self.X0, DBCRangeMin, DBCRangeMax, v, rotCenter, rotAxis, angVelDeg, scaleVel, self.DBC,
                           DBCMotion, Vector4i(0, 0, 1000000000, -1))
        self.DBCMotionList.append((durationStart, durationEnd, DBCMotion))

    def set_DBC_wrt_X(self, DBCRangeMin, DBCRangeMax, v, rotCenter, rotAxis, angVelDeg, scaleVel=0., durationStart=0,
                      durationEnd=1e30):
        DBCMotion = Storage.V2iV2dV2dV2dSdSdStorage() if self.dim == 2 else Storage.V2iV3dV3dV3dSdSdStorage()
        FEM.Init_Dirichlet(self.X, DBCRangeMin, DBCRangeMax, v, rotCenter, rotAxis, angVelDeg, scaleVel, self.DBC,
                           DBCMotion, Vector4i(0, 0, 1000000000, -1))
        self.DBCMotionList.append((durationStart, durationEnd, DBCMotion))
        ...
        if self.useNewton:
            if self.EIPC:
                self.PNIterCount += FEM.TimeStepper.ImplicitEuler.Advance_One_Step_EIPC(self.Elem, self.DBC, \
                                                                                        ...
                                                                                        self.PNTol, self.withCollision,
                                                                                        self.dHat2, self.kappa, self.mu,
                                                                                        self.epsv2, self.bdstiff,
                                                                                        self.bdK, \
                                                                                        self.staticSolve,
                                                                                        self.withShapeMatching,
                                                                                        self.output_folder,
                                                                                        self.current_frame, self.X,
                                                                                        self.X0, self.nodeAttr,
                                                                                        self.elemAttr, self.elasticity)
            else:
                self.PNIterCount += FEM.TimeStepper.ImplicitEuler.Advance_One_Step(self.Elem, self.DBC, \
                                                                                   self.gravity, self.dt, self.PNTol,
                                                                                   self.withCollision, self.dHat2,
                                                                                   self.kappa, self.mu, self.epsv2,
                                                                                   self.bdstiff, self.bdK, \
                                                                                   self.staticSolve,
                                                                                   self.withShapeMatching,
                                                                                   self.output_folder,
                                                                                   self.current_frame, self.X, self.X0,
                                                                                   self.nodeAttr, self.elemAttr,
                                                                                   self.elasticity, self.Extra_Mesh)
        else:
    ...
    def write_image(self, index):
        if self.dim == 2:
            tri = MeshIO.Taichi_TriMesh_Obj(self.X, self.Elem)
            tri = tri * self.camera_scale + self.camera_offset
            self.gui.triangles(tri[0, :, :], tri[1, :, :], tri[2, :, :], color=0xE6E229)
            DBC = MeshIO.Taichi_DBC_Obj(self.DBC)
            DBC = DBC * self.camera_scale + self.camera_offset
            self.gui.circles(DBC, radius=2)
            self.gui.circle([self.mouse_x, self.mouse_y], radius=10)
        else:
            self.scene.input(self.gui)
            data = MeshIO.Taichi_SurfaceMesh_Obj(self.X, self.TriVI2TetVI, self.Tri)
            data = data * self.camera_scale + self.camera_offset
            self.model.set_face_verts(data.astype(np.float32))
            self.scene.render()
            self.gui.set_image(self.scene.img)
            self.gui.text("Obj: Armadillo2.5K", (0.02, 0.98), color=0xFFFFFF, font_size=27)
            self.gui.text("Mode: Push", (0.02, 0.95), color=0xFFFFFF, font_size=27)
            self.gui.text("Total PN Iter#: {:d}".format(self.PNIterCount), (0.02, 0.92), color=0xFFFFFF, font_size=27)
            self.gui.text("Contact#: {:d}".format(Get_Parameter("Contact_Num", 0)), (0.02, 0.89), color=0xFFFFFF,
                          font_size=27)
        if self.interactive:
            self.gui.show()
        else:
    ...
    def write(self, frame_idx):
        ...
        if self.dim == 2:
            ...
            MeshIO.Write_Surface_TriMesh_Obj(self.X, self.TriVI2TetVI, self.Tri, self.Xt, fn, self.WorkIndex)
        if self.hero_counter:
            MeshIO.Write_Surface_TriMesh_Obj_Regional(self.X, self.TriVI2TetVI, self.Tri, self.Xt,
                                                      self.output_folder + "hero_" + str(frame_idx) + ".obj",
                                                      self.hero_counter,self.WorkIndex,self.translate,self.rotCenter,
                                                      self.rotAxis,self.rotDeg,self.scale)

    def write_com(self):
        FEM.TimeStepper.ImplicitEuler.Write_COM(self.X, self.nodeAttr, self.output_folder + "com.txt")

    def generate_gif(self):
        return super().generate_gif()
